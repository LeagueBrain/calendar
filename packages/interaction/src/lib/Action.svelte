	...
	export function dragTimeGrid(eventToDrag, el, jsEvent, resourcesStore, allDay, resize) {
		if (!action && jsEvent.isPrimary) {
			...
			if (allDay && (!resourcesStore || !$datesAboveResources)) {
				offsetX =
					floor((jsEvent.clientX - colRect.left) / colRect.width) -
					col -
					(resourceCol || 0) * $_viewDates.length;
			}
	...
	function move(jsEvent) {
		...
		if (
			...
			if (view === VIEW_TIME_GRID) {
				...
				delta = createDuration({
					days:
						($_viewDates[newRow * cols + newCol] - $_viewDates[row * cols + col]) /
							1000 /
							60 /
							60 /
							24 -
			...
			if (resizing()) {
				...
				if (_viewResources) {
					$_iEvents[0].resourceIds = event.resourceIds.filter(
						(id) => id !== $_viewResources[resourceCol].id
					);
					...
					if (is_function(callback)) {
						...
						let info = resizing()
							...
							: {
									delta,
									oldResource:
										resourceCol !== newResourceCol ? $_viewResources[resourceCol] : undefined,
									newResource:
										resourceCol !== newResourceCol ? $_viewResources[newResourceCol] : undefined
							  };
						callback(
							Object.assign(info, {
								event: toEventWithLocalDates(event),
