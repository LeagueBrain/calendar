...
import { derived2 } from '@event-calendar/common/internal';
import { createEvents } from '@event-calendar/common/internal';
import { createView } from '@event-calendar/common/internal';
...
export function view(state) {
	return derived2([state.view, state._viewTitle, state._currentRange, state._activeRange], (args) =>
		createView(...args)
	);
}
...
export function events(state) {
	...
	derived(
		...
		(values, set) =>
			debounce(
				() => {
					let [$events, $eventSources, $_activeRange, $_fetchedRange, $lazyFetching, $loading] =
						values;
					...
					if (
						...
						for (let source of $eventSources) {
							if (is_function(source.events)) {
								...
								// Prepare params
								let params = is_function(source.extraParams)
									? source.extraParams()
									: Object.assign({}, source.extraParams);
