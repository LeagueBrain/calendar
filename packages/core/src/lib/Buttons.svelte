<script>
	...
	export let buttons;
	let { _currentRange, _viewTitle, buttonText, date, duration, hiddenDays, theme, view } =
		getContext('state');
...
{#each buttons as button}
	{#if button == ''}{:else if button == 'title'}
		<h2 class={$theme.title}>{$_viewTitle}</h2>
	{:else if button == 'prev'}
		<button class="{$theme.button} ec-{button}" on:click={prev}
			><i class="{$theme.icon} ec-{button}" /></button
		>
	{:else if button === 'next'}
		<button class="{$theme.button} ec-{button}" on:click={next}
			><i class="{$theme.icon} ec-{button}" /></button
		>
