<script>
	...
	$: {
		...
		let bgColor =
			event.backgroundColor || $_resBgColor(event) || $eventBackgroundColor || $eventColor;
		style =
			`top:${top}px;` +
		...
		if (display === 'auto' || display === 'ghost') {
			style +=
				...
				`width:${
					(100 / chunk.group.columns.length) * 0.5 * (1 + chunk.group.columns.length - chunk.column)
				}%;`;
		}
	...
	function createDragHandler(resize) {
		return (jsEvent) =>
			$_interaction.action.dragTimeGrid(event, el, jsEvent, _viewResources, false, resize);
	}
</script>
...
>
	...
	<svelte:component this={$_interaction.resizer} {event} on:pointerdown={createDragHandler(true)} />
</div>
