<script>
	...
	import { hasYScroll, setContent } from '@event-calendar/common/internal';
	...
	$: if (el && $_viewDates) {
		el.scrollTop =
			(($scrollTime.seconds - timeLimitMin) / $slotDuration.seconds - 0.5) * $slotHeight;
	}
...
<div bind:this={el} class="{$theme.body}{compact ? ' ' + $theme.compact : ''}" on:scroll={$_scroll}>
	<div class={$theme.content}>
		<div class={$theme.sidebar}>
			<div class={$theme.sidebarTitle} use:setContent={allDayText} />
			{#each $_times as time}
